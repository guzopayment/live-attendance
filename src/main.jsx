...
const SOCKET_URL =
  import.meta.env.VITE_SOCKET_URL || API.replace(/\/api\/?$/, "");
  ...
  return (
    <div className="ceremonyClock" aria-label="Current Addis Ababa time">
      <div className="clockFace">
        ...
        <span
          className="clockHand hourHand"
          style={{ transform: `rotate(${hourAngle}deg)` }}
        />
        <span
          className="clockHand minuteHand"
          style={{ transform: `rotate(${minuteAngle}deg)` }}
        />
        <span
          className="clockHand secondHand"
...
function OrganizationGenderCards({
  ...
  const organizations = useMemo(() => {
    const normalizeSex = (value) => {
      ...
      if (["ሴት", "female", "f", "woman", "women"].includes(sex))
        return "female";
      return "unknown";
    };
    ...
    for (const row of allRows) {
      const name = String(row?.organization ?? "")
        .replace(/\s+/g, " ")
        .trim();
    ...
    if (map.size > 0) {
      return [...map.values()].sort(
        (a, b) => b.registered.total - a.registered.total,
      );
    }
    ...
    return rows
      .filter((item) =>
        String(item?.organization || item?.organizationName || "").trim(),
      )
      .map((item) => ({
        ...item,
  ...
  const latestParticipant = isRecentOrganization
    ? recent?.latestParticipant
    : null;
  ...
  const metric = (bucket) => (
    <div className="bigAnalyticsMetric">
      ...
      <div className="bigAnalyticsProgress">
        <span
          style={{
            width: `${Math.min(100, Math.max(0, bucket.malePercent))}%`,
          }}
        />
      </div>
      ...
      <div className="bigAnalyticsProgress">
        <span
          style={{
            width: `${Math.min(100, Math.max(0, bucket.femalePercent))}%`,
          }}
        />
      </div>
      ...
      <div className="bigAnalyticsCard">
        <div className="bigAnalyticsTop">
          <div className="bigAnalyticsTitle">
            ...
            <h2>{selectedName}</h2>
            <p>
              Organization-by-organization live attendance • changes
              automatically every 7 seconds
            </p>
          </div>

          <div
            className="bigAnalyticsRotation"
            aria-label="Organization rotation"
          >
            <span>
              {organizations.length
                ? `${activeIndex + 1} / ${organizations.length}`
                : "0 / 0"}
            </span>
        ...
        {selected ? (
          ...
          <div className="bigAnalyticsEmpty">
            ድርጅት አልተገኘም / No organization data available.
          </div>
        )}
      </div>
    </section>
      ...
      <main className="liveMain">
        ...
        <div className="rateBanner">
          ...
          <div className="rateBar">
            <div
              style={{
                width: `${Math.min(100, Math.max(0, data.presentPercent))}%`,
              }}
            />
          </div>
          <small>
            {data.totalPresent} of {data.totalRegistered} participants have
            checked in
          </small>
        </div>
...
function OperatorPage() {
  ...
  const organizations = useMemo(() => {
    const set = new Set(
      (summary?.absent || []).map((row) => row.organization).filter(Boolean),
    );
  ...
  if (!authenticated)
    return <OperatorLogin onLogin={() => setAuthenticated(true)} />;
    ...
    <div className="operatorPage">
      <header className="operatorHeader">
        <div>
          ...
          <h1>Attendance Operator</h1>
          <p>
            Search / filter to verify whether a participant has already checked
            in.
          </p>
        </div>
        <div className="operatorActions">
          <a className="secondaryButton" href="/">
            <Monitor size={17} /> Live Screen
          </a>
        ...
        <section className="searchCard">
          <div className="searchTitle">
            ...
            <div>
              ...
              <p>Search by Amharic name, organization, or phone.</p>
            </div>
          </div>
        ...
        <section className="operatorResults">
          <div className="resultsHead">
            <div>
              ...
              <p>{data?.length || 0} participant(s)</p>
            </div>
            <button className="refreshButton" onClick={search}>
